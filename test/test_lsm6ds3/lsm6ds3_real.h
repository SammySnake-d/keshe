...
void test_real_sampling_stability() {
    ...
    // 计算标准差
    float avg = 0;
    for (int i = 0; i < 10; i++) avg += pitches[i];
    avg /= 10;
...
/**
 * @brief Real测试：中断引脚（可选）
 */
void test_real_interrupt_pin() {
    Serial.println("\n[TEST] Real: 中断引脚（可选）");
    
    pinMode(PIN_LSM6DS3_INT1, INPUT);
    int state = digitalRead(PIN_LSM6DS3_INT1);
    
    Serial.printf("  INT1 引脚状态: %s\n", state ? "HIGH" : "LOW");
    Serial.println("  ℹ️  如已配置数据就绪中断，此引脚应周期性跳变");
    
    // 注：此测试仅检查引脚可读，实际中断功能需要配置寄存器
}
